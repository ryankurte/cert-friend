...
type RSAOptions struct {
	KeyBits int `long:"rsa-bits" description:"RSA private key length" default:"4096" yaml:",omitempty"`
}
...
type CreateKeyOptions struct {
	...
	UsePKCS8     bool   `long:"pkcs8" description:"Use PKCS8 encoding for private key files"`
...
func (o *CreateKeyOptions) Interactive() {
	log.Printf(
		"We have a choice between RSA and ECDSA:\n" +
			"RSA is a reasonably old algorithm based on prime factorisation, and is well supported almost everywhere. RSA keys should be > 2096 bits, and operations are relatively slow on embedded systems\n" +
			"	For more information see: https://en.wikipedia.org/wiki/RSA_(cryptosystem)\n" +
			"ECDSA is a more modern elliptic curve based algorithm. ECDSA keys can be shorter (256-512bits) and performance on embedded systems is slightly better, however support is not quite so widespread\n" +
